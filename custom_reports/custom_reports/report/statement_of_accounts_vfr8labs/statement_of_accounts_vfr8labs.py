...
def execute(filters=None):
	# filters["party_name"] = get_party_name(filters.get("party_type"), filters.get("party"))
...
# 					else frappe.db.get_value(filters.party_type, filters.party, "default_currency")
# 				)
...
# 	print("gle_currency:", gle_currency)
# 	return filters
...
#     # # Set a default party_type if not present in filters
#     # filters.setdefault("party_type", "Customer")  # Replace "Customer" with the default party_type you want to use
    
#     # Get account currency from GL Entry
...
#     if gle_currency:
#         account_currency = gle_currency[0].get("account_currency")
#     else:
#         # Set account currency to company currency if not found in GL Entry
#         account_currency = filters.company_currency

#     filters["account_currency"] = account_currency

#     # Set presentation currency if account_currency is different from company_currency
#     if filters.account_currency != filters.company_currency and not filters.presentation_currency:
#         filters.presentation_currency = filters.account_currency
        ...
        if filters.get("account"):
            ...
            if party_type and party:
                ...
                if gle_currency:
                    ...
                    account_currency = (
                        None
                        if party_type in ["Employee", "Shareholder", "Member"]
                        else frappe.db.get_value(party_type, party, "default_currency")
                    )
...
def get_gl_entries(filters, accounting_dimensions):
	...
	gl_entries = frappe.db.sql(
			...
			select
				...
				inv.outstanding_amount, inv.job_no,	inv.awb_bl_no, inv.mawb_obl_no, inv.vessel, inv.due_date, inv.invoice_date
			from `tabGL Entry` gl
			left join `tabSales Invoice` inv on inv.name = gl.against_voucher and gl.against_voucher_type = 'Sales Invoice'
...
#     if party_type and party:
#         party_name = frappe.db.get_value(party_type, party, "name")
#         return party_name
#     return None
...
def get_conditions(filters):
	...
	# if filters.get("omit_outstanding"):
...
def get_result_as_list(data, filters):
    ...
    for d in data:
        ...
        balance_in_account_currency = get_balance(d, balance_in_account_currency, "debit_in_account_currency", "credit_in_account_currency")
...
def get_columns(filters):
	...
	if filters.get("company") == "PT. ASIA GROW LOGISTICS":
			# Columns.extend takes only 1 argument, therefore we need to add the square brackets to group both columns together.
			columns.extend([{
			"label": _("BL No."),
			"fieldname": "awb_bl_no",
			"width": 130,
	...
	if filters.get("company") == "WPC Marine & Offshore Services Pte Ltd":
			columns.append({
				"label": _("Vessel"),
				"fieldname": "vessel",
				"width": 130,
