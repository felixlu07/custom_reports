...
from frappe.utils import add_to_date, cint, cstr, flt, getdate, nowdate
...
class ReceivablePayableReport(object):
	...
	def build_data(self):
		cumulative_outstanding = 0  # initialize cumulative outstanding variable
		# selected_invoice_date = None
		# if self.filters.get("invoice_date"):
		# 	selected_invoice_date = getdate(self.filters.get("invoice_date"))
		# set outstanding for all the accumulated balances, as we can use this to filter out invoices without outstanding
		for key, row in self.voucher_balance.items():
			invoice_date = self.invoice_details.get(row.voucher_no, {}).get("invoice_date")
			...
			# calculate cumulative outstanding and set value for new column
			cumulative_outstanding += row.outstanding
	...
	def append_row(self, row):
		...
		self.set_ageing(row)
		self.set_monthly_ageing(row)
		self.calculate_due_invoice_diff(row)
	...
	def get_invoice_details(self):
		...
		if self.party_type == "Customer":
			si_list = frappe.db.sql(
				"""
				select name, due_date, po_no, invoice_date
	...
	def set_ageing(self, row):
		if self.filters.ageing_based_on == "Due Date":
			...
			# when ageing viewed by due date. Previously it was 'or row.posting_date
			entry_date = row.due_date or row.invoice_date or row.posting_date
		elif self.filters.ageing_based_on == "Supplier Invoice Date":
			entry_date = row.bill_date
		else:
			# Previously it was 'row.posting_date
			entry_date = row.invoice_date or row.posting_date
	...
	def set_monthly_ageing(self, row):
		# Based on your settings, choose the right date
		if self.filters.ageing_based_on == "Due Date":
			entry_date = row.due_date or row.invoice_date or row.posting_date
		elif self.filters.ageing_based_on == "Supplier Invoice Date":
			entry_date = row.bill_date
		...
		self.get_monthly_ageing_data(entry_date, row)

		# Zero out ageing buckets if entry date is in the future
		if getdate(entry_date) > getdate(self.filters.report_date):
			for i in range(5):
				row["month_range" + str(i + 1)] = 0.0

		# Total due is the sum of all ranges
		row.total_due = sum(row["month_range" + str(i + 1)] for i in range(5))

	def get_monthly_ageing_data(self, entry_date, row):
		# Initialize the ranges if they haven't been initialized already
		if not hasattr(self, 'month_buckets'):
			self.month_buckets = {
				getdate(add_to_date(self.age_as_on, months=-i)).month: i + 1 
				for i in range(5)
			}
		...
		# Extract the month from the entry_date
		entry_month = getdate(entry_date).month

		# Initialize the row ranges
		for i in range(5):
			row["month_range" + str(i + 1)] = 0.0

		# Check if the entry_month is in the month_buckets
		if entry_month in self.month_buckets:
			index = self.month_buckets[entry_month]
			row["month_range" + str(index)] = row.outstanding
		else:
			# If the entry_month is not in the month_buckets, it belongs to the catch-all bucket
			catch_all_bucket = getdate(add_to_date(self.age_as_on, months=-4)).month
			index = self.month_buckets[catch_all_bucket]
			row["month_range" + str(index)] = row.outstanding

	
	def calculate_due_invoice_diff(self, row):
		if row.due_date == row.invoice_date:
			row.invoice_due_diff = "COD"
		else:
			difference = (getdate(row.due_date) - getdate(row.invoice_date)).days
			row.invoice_due_diff = f'{difference} days'
		return row
	...
	def get_ple_entries(self):
		...
		if self.filters.get("group_by_party"):
			if self.filters.get("posting_date_sort"):
				query = query.orderby(self.ple.party, self.ple.posting_date)
			else:
				query = query.orderby(self.ple.party, si.invoice_date)
		else:
			if self.filters.get("posting_date_sort"):
				query = query.orderby(self.ple.posting_date, self.ple.party)
			else:
				query = query.orderby(si.invoice_date, self.ple.party)
	...
	def get_columns(self):
		...
		self.add_column(_("Invoice Date"), fieldname="invoice_date", fieldtype="Date")
		self.add_column(
		...
		self.add_column(_("Balance"), fieldname="cumulative_outstanding")

		self.setup_ageing_columns()
		self.setup_monthly_ageing_columns()
		...
		if self.filters.show_top:
			self.add_column(label=_("TOP"), fieldname="invoice_due_diff", fieldtype="Int", width=80),
	...
	def setup_monthly_ageing_columns(self):
		# for charts
		self.monthly_ageing_column_labels = []
		
		for i in range(5):
			# Get the month name
			month_name = getdate(add_to_date(self.age_as_on, months=-i)).strftime('%B\'%y')

			 # Create the column label
			if i == 4:
				label = "{month}+".format(month=month_name)
			else:
				label = "{month}".format(month=month_name)

			# Add the column to the table
			self.add_column(label=label, fieldname="month_range" + str(i + 1))
			
			# Add the label to the ageing_column_labels list
			self.monthly_ageing_column_labels.append(label)
