...
def execute(filters=None):
	...
	for d in data:
		if (
		(d.clearance_date == "" or d.clearance_date is None) or
		(isinstance(d.clearance_date, str)
		and datetime.fromisoformat(d.clearance_date) <= filters["report_date"])
		):
...
def get_columns():
	return [
		...
		{
			"fieldname": "reconciled_against",
			"label": _("Reconciled Against"),
			"fieldtype": "Link",
...
def get_journal_entries(filters):
	return frappe.db.sql(
		...
		select "Journal Entry" as payment_document, jv.posting_date,
			...
			jv.cheque_no as reference_no, jv.cheque_date as ref_date, jv.clearance_date, jvd.account_currency, jv.reconciled_against
		from
			`tabJournal Entry Account` jvd, `tabJournal Entry` jv
		where jvd.parent = jv.name and jv.docstatus=1
			and jvd.account = %(account)s and jv.posting_date <= %(report_date)s
			and (ifnull(jv.clearance_date, '4000-01-01') > %(report_date)s or jv.clearance_date is not null)
...
def get_payment_entries(filters):
	return frappe.db.sql(
		...
		select
			...
			if(paid_to=%(account)s, paid_to_account_currency, paid_from_account_currency) as account_currency, reconciled_against
		from `tabPayment Entry`
		where
			(paid_from=%(account)s or paid_to=%(account)s) and docstatus=1
			and posting_date <= %(report_date)s
			and (ifnull(clearance_date, '4000-01-01') > %(report_date)s or clearance_date is not null)
	""",
		filters,
		as_dict=1,
	)
	...
	return frappe.db.sql(
		"""
			select
				...
				account.account_currency, 0 as credit, sip.reconciled_against
			from `tabSales Invoice Payment` sip, `tabSales Invoice` si, `tabAccount` account
			where
				sip.account=%(account)s and si.docstatus=1 and sip.parent = si.name
				and account.name = sip.account and si.posting_date <= %(report_date)s 
				and ifnull(sip.clearance_date, '4000-01-01') > %(report_date)s
			order by
