...
def get_gl_entries(filters):
	...
	select * from
		(select
			...
			account_currency, voucher_type, voucher_no, remarks, is_opening
		from `tabGL Entry` gl left join `tabConsolidation Item` c on c.party = gl.party and c.party_type = gl.party_type
		where company=%(company)s
		and gl.party_type IN ('Customer', 'Supplier')
		group by account_currency, voucher_type, voucher_no) tb
		where {conditions}
