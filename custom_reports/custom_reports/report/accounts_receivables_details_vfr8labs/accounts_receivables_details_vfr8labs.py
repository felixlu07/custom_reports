...
class ReceivablePayableReport(object):
	...
	def update_voucher_balance(self, ple):
		...
		# Set to always use account currency
		amount = ple.amount_in_account_currency
	...
	def get_invoice_details(self):
		...
		if self.party_type == "Customer":
			si_list = frappe.db.sql(
				"""
				select name, due_date, po_no, job_no
		...
		if self.party_type == "Supplier":
			for pi in frappe.db.sql(
				"""
				select name, due_date, bill_no, bill_date, job_no
	...
	def set_party_details(self, row):
		...
		# Always set the row.currency to row.account_currency regardless of any filters
	...
	def get_columns(self):
		...
		self.add_column(label=_("Job No"), fieldname="job_no", fieldtype="Data")
		if self.party_type == "Supplier":
